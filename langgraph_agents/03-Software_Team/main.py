...
class GraphState(TypedDict):
    messages: Annotated[list, add_messages] # the add_messages function is used to add messages to the state from each node
...
# Draw the graph
try:
    graph.get_graph(xray=True).draw_mermaid_png(output_file_path="langgraph_agents/03-Software_Team/graph.png")
except Exception:
    pass
...
def main_loop():
    ...
    while True:
        ...
        print("User: ", response["messages"][-8].content)
